...
def _sham_booking(self,
    ...
    while remaining > 0 and press_round < MAX_PRESS_ROUND:
        ...
        LOG.info(f"TG本轮押位人数[{seat_count}]，目标舱位[{target_cabin}]，剩余[{remaining}]", "押位流程")
        ...
        try:
            ...
            press_round += 1
            LOG.info(f"TG押位成功PNR[{round_order.pnr}]，累计人数[{len(all_passengers)}]", "押位流程")
            ...
            if pnr_list and e.code in [
                ...
                LOG.info(f"TG后续押位无库存，返回已押位PNR，错误[{e.message}]", "押位流程")
                break
            if pnr_list:
                LOG.error(f"TG后续押位失败，返回已押位PNR，错误[{e.message}]", "押位流程")
                break
            raise
        except Exception as e:
            if pnr_list:
                LOG.error(f"TG后续押位异常，返回已押位PNR，错误[{e}]", "押位流程")
                break
            raise

    if not pnr_list:
        raise ServiceError(ServiceStateEnum.BUSINESS_ERROR, "TG占座失败，未生成任何PNR")
