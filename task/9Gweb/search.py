...
LOG = log_util.LogUtil("sunPhuQuocAirwaysWebSearch")


def _web_date(value: Optional[str]) -> Optional[str]:
    if not value:
        return None
...
def _run_search(service: WebService, search_data: RequestSearchTaskDataModel):
    return service.search(
        dep_airport=search_data.dep_airport,
        arr_airport=search_data.arr_airport,
        dep_date=_web_date(search_data.dep_date),
        ret_date=_web_date(search_data.ret_date),
