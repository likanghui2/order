...
if [ "$TASK_TYPE" == "search" ] || [ "$TASK_TYPE" == "verify" ]; then
    exec celery -A task.AKapp.search worker -Q AKapp-$TASK_TYPE --concurrency=$CONCURRENCY -P threads --without-heartbeat --without-gossip --without-mingle --task-events
