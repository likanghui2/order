...
class FormData(list):
    ...
    def set(self, key, value):
        ...
        new_data = FormData()
        inserted = False
        for name, item_value in self:
            if name != key:
                new_data.append((name, item_value))
                continue
            if not inserted:
                new_data.extend((key, v) for v in values)
                inserted = True
        if not inserted:
            new_data.extend((key, v) for v in values)
        self[:] = new_data
    ...
    def update(self, other=None, **kwargs):
        items = []
        if other:
            items.extend(other.items() if hasattr(other, "items") else other)
    ...
    def remove(self, *keys):
        ...
        self[:] = [(name, value) for name, value in self if name not in remove_keys]

    def insert_after(self, anchor_key, key, value):
        ...
        if insert_at is None:
            self.extend((key, v) for v in values)
            return
        for offset, item_value in enumerate(values):
            self.insert(insert_at + offset, (key, item_value))
...
class Utils:
    ...
    @classmethod
    def extract_forms_data(cls, html_content):
        doc = pq(html_content)
        return [
            cls.get_form_data(form, preserve_duplicate=True, browser_successful=True)
            for form in doc("form").items()
        ]

    @classmethod
    def extract_inputs_data(cls, html_content):
        doc = pq(html_content)
        form_data = {}
        for input_tag in doc("input").items():
            input_name = input_tag.attr("id") or input_tag.attr("name") or ""
    ...
    @classmethod
    def get_form_data(cls, form, preserve_duplicate=False, browser_successful=False):
        # action = form.attrib["action"]
        action = form.attr("action")
        # print(action)
        inputs = form("input, select, textarea") if browser_successful else form("input")
        form_data = FormData() if preserve_duplicate else {}
        for input_tag in inputs.items():
            # input_type = input_tag.attr("type")
            entries = (
                cls.__successful_control_entries(input_tag)
                if browser_successful
                else [(input_tag.attr("name") or "", input_tag.attr("value") or "")]
            )
    ...
    @staticmethod
    def __successful_control_entries(control):
        if control.attr("disabled") is not None:
            return []
        control_name = control.attr("name") or ""
        if not control_name:
            return []
        tag_name = getattr(control[0], "tag", "").lower()
        if tag_name == "input":
            input_type = (control.attr("type") or "text").lower()
            if input_type in {"button", "reset", "submit", "image", "file"}:
                return []
            if input_type in {"checkbox", "radio"} and control.attr("checked") is None:
                return []
            return [(control_name, control.attr("value") or "")]
        if tag_name == "select":
            selected_options = [
                option for option in control("option").items()
                if option.attr("selected") is not None and option.attr("disabled") is None
            ]
            if not selected_options and control.attr("multiple") is None:
                selected_options = [
                    option for option in control("option").items()
                    if option.attr("disabled") is None
                ][:1]
            return [
                (control_name, option.attr("value") if option.attr("value") is not None else option.text())
                for option in selected_options
            ]
        if tag_name == "textarea":
            return [(control_name, control.val() or control.text() or "")]
        return []
    ...
    def parse_book_order(booking_info: dict, response_order_data: ResponseOrderInfoModel):
        for i in response_order_data.passengers:
            for j in booking_info['passengerInfos']:
                if i.last_name.lower() == j['surname'].lower() and i.first_name.lower() == j['givenName'].lower():
