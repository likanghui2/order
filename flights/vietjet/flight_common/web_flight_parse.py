...
class FlightParser:
    ...
    @classmethod
    def journey_info_parser(cls, flight_data: dict, baggage_data: dict)-> List[FlightJourneyModel]:
        result_data_list = []
        for index, (key, itinerarie) in enumerate(flight_data["travelOption"].items(), start=1):
            for fare in itinerarie:
                ...
                if len(segments) != 1:
                    continue
                bundles = cls.bundle_parser(fare['fareOptions'], baggage_info=baggage_data, segments=segments)
                if not bundles:
                    continue
    ...
    @classmethod
    def bundle_parser(cls, data: List[dict], baggage_info, segments: List[FlightSegmentModel]) -> List[FlightBundleModel]:
        bundle_list = []
        for bundle in data:
            ...
            for i in bundle['fareCharges']:
                ...
                if i['chargeType']['code'] == 'FA':
                    ...
                    if i['passengerApplicability']['child']:
                        chd_fare = money
                elif i['chargeType']['code'] != 'FA' and i['description'] != 'Seat Assignment':
                    if i['passengerApplicability']['adult']:
                        adt_tax += money
                    if i['passengerApplicability']['child']:
                        chd_tax += money
            ...
            temp_ssr_list = []
            product_tag = bundle['fareType']['description']
            if baggage_info:
                baggage_datas = cls.__extract_brand_data(baggage_info, product_tag, segments)
            ...
            bundle_list.append(FlightBundleModel(
                ...
                ssrInfo=ssr_info,
                code=bundle['fareType']['description'],
                cabinLevel=bundle["cabinClass"]["code"],
                cabin=bundle['bookingCode']['code'],
                fareKey=bundle['bookingKey'],
                productTag=bundle['fareType']['description'],
                seat=seats,
